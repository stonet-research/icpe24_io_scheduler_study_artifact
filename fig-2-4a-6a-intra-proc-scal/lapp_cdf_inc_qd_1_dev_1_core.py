...
FIO_OPTIONS = '--size=100% --bs=4kb --norandommap=1 --group_reporting=1 --direct=1 --rw=randread --allow_file_create=0 --output-format=json'.format(
...
if config['run']:
    ...
    for cur_qd in num_qd:
        for cur_sched in schedulers:
            output_file = 'qd_{}_dev_{}_sche_{}.txt'.format(
                cur_qd, 1, cur_sched)
            ...
            cpu_option = '--cpus_allowed=' + str(cpu_allowed)
            latency_log_option = '--write_lat_log=' + latency_log_prefix
            fio_option_cur = ' '.join(
                [scheduler_option, qd_option, cpu_option])
    ...
    for cur_sched in schedulers:
        ...
        for cur_num_qd in num_qd:
            ...
            for cur_lat_name, cur_lat_log in zip(
                ...
                cur_latencies = []
                for proc_index in range(1,2):
                    cur_lat_log_file = latency_log_prefix + '_' + cur_lat_name + '.1.log'
