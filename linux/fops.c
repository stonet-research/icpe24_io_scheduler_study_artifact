...
static int force_sync_submission __read_mostly;
module_param(force_sync_submission, int, 0644);
MODULE_PARM_DESC(force_sync_submission, "Force sync submission of bio");
...
static ssize_t blkdev_direct_IO(struct kiocb *iocb, struct iov_iter *iter)
{
	...
	if (likely(nr_pages <= BIO_MAX_VECS)) {
		if (force_sync_submission == 1) {
			// If force_sync_submission is set, then always use __blkdev_direct_IO_simple, as Linux 5.15.43
			return __blkdev_direct_IO_simple(iocb, iter, nr_pages);
		} else if (force_sync_submission == 2) {
