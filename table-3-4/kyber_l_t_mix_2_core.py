...
# L-app about 80us. T-4KiB write about 12ms
parameter_value_ticks_read = [
    '0', '50us', '100us', '500us', '10ms', '100ms', '1s'
]
parameter_values_read = [MS_0, US_50, US_100, US_500, MS_10, MS_100, S_1]

parameter_value_ticks_write = ['0', '100us', '10ms', '20ms', '100ms', '1s']
parameter_values_write = [MS_0, US_100, MS_10, MS_20, MS_100, S_1]

parameter_value_ticks = []
parameter_values = []

for cur_read_tick, cur_read_value in zip(parameter_value_ticks_read,
                                         parameter_values_read):
    for cur_write_tick, cur_write_value in zip(parameter_value_ticks_write,
                                               parameter_values_write):
        parameter_value_ticks.append((cur_read_tick, cur_write_tick))
        parameter_values.append((cur_read_value, cur_write_value))
...
workload_fio_options = [
    '--name=1 --iodepth=1 --rw=randread --cpus_allowed=1 --name=2  --iodepth=128 --rw=randwrite --cpus_allowed=2'
]

# settings
results_root = 'results_kyber_l_t_mix'
...
FIO_OPTIONS = '--size=100% --norandommap=1 --group_reporting=1 --direct=1 --allow_file_create=0 --output-format=json --bs=4k'
...
avg_lat_read_results = {}
avg_lat_write_results = {}
p99_lat_read_results = {}
p99_lat_write_results = {}
iops_read_results = {}
iops_stddev = {}
iops_write_results = {}
iops_write_stddev = {}
...
for cur_workload_name in workload_name:
    avg_lat_read_results[cur_workload_name] = {}
    avg_lat_write_results[cur_workload_name] = {}
    p99_lat_read_results[cur_workload_name] = {}
    p99_lat_write_results[cur_workload_name] = {}
    ...
    for cur_para_ticks in parameter_value_ticks:
        ...
        for cur_rep in range(rep):
            ...
            cur_conf_read_lat.append(j_res['read:lat_ns:mean'] / 1000)
            cur_conf_write_lat.append(j_res['write:lat_ns:mean'] / 1000)
            cur_conf_read_p99.append(
                j_res['read:clat_ns:percentile:99.000000'] / 1000)
            cur_conf_write_p99.append(
        ...
        iops_read_results[cur_workload_name][
            cur_para_ticks] = cur_conf_read_iops
        iops_write_results[cur_workload_name][
            cur_para_ticks] = cur_conf_write_iops
        avg_lat_read_results[cur_workload_name][
            cur_para_ticks] = cur_conf_read_lat
        avg_lat_write_results[cur_workload_name][
            cur_para_ticks] = cur_conf_write_lat
        p99_lat_read_results[cur_workload_name][
            cur_para_ticks] = cur_conf_read_p99
        p99_lat_write_results[cur_workload_name][
            cur_para_ticks] = cur_conf_write_p99
...
for cur_workload_name in workload_name:
    ...
    for cur_para_ticks in parameter_value_ticks:
        ...
        for cur_rep in range(rep):
            ...
            f.close()
        avg_qd_results[cur_workload_name][cur_para_ticks] = (
            np.average(cur_conf_read_depth), np.average(cur_conf_write_depth))
        std_qd_results[cur_workload_name][cur_para_ticks] = (
            np.std(cur_conf_read_depth), np.std(cur_conf_write_depth))
        qd_results[cur_workload_name][cur_para_ticks] = cur_conf_qd_results
        qd_results_sec[cur_workload_name][
            cur_para_ticks] = cur_conf_qd_results_sec
...
def avg_lat_2_string(avg_latencies):
    return str(round(np.average(avg_latencies), 1))


def avg_lat_2_string_ms(avg_latencies):
    return str(round(np.average(avg_latencies) / 1000, 1))


# print("Read average latency: ")
# for cur_workload in workload_name:
#     print(workload_name)
#     print(' & '.join([' '] + parameter_value_ticks_write) + ' \\\\')
#     print('\\hline')
#     for cur_read_lat_tick in parameter_value_ticks_read:
#         curline = str(cur_read_lat_tick)
#         for cur_write_lat_tick in parameter_value_ticks_write:
#             tick_key = (cur_read_lat_tick, cur_write_lat_tick)
#             curline += ' & {} ({}-{})'.format(
#                 avg_lat_2_string(avg_lat_read_results[cur_workload][tick_key]),
#                 int(avg_qd_results[cur_workload][tick_key][0]),
#                 int(avg_qd_results[cur_workload][tick_key][1]))
#         print(curline + ' \\\\')
# print("Read Latency end.")
# print("#################")
print('Read p99 latency.')
for cur_workload in workload_name:
    print(workload_name)
    print(' & '.join([' '] + parameter_value_ticks_write) + ' \\\\')
    print('\\hline')
    for cur_read_lat_tick in parameter_value_ticks_read:
        ...
        for cur_write_lat_tick in parameter_value_ticks_write:
            ...
            curline += ' & {}-{} ({}-{})'.format(
                avg_lat_2_string_ms(
                    p99_lat_read_results[cur_workload][tick_key]),
                avg_lat_2_string_ms(
                    p99_lat_write_results[cur_workload][tick_key]),
                int(avg_qd_results[cur_workload][tick_key][0]),
                int(avg_qd_results[cur_workload][tick_key][1]))
        print(curline + ' \\\\')
print("Read Latency end.")
print('Read p99 latency end.')
print('#################')
print("Write IOPS")
for cur_workload in workload_name:
    print(workload_name)
    ...
    for cur_read_lat_tick in parameter_value_ticks_read:
        ...
        for cur_write_lat_tick in parameter_value_ticks_write:
            tick_key = (cur_read_lat_tick, cur_write_lat_tick)
            curline += ' & {} ({}-{})'.format(
                avg_lat_2_string(iops_write_results[cur_workload][tick_key]),
                int(avg_qd_results[cur_workload][tick_key][0]),
                int(avg_qd_results[cur_workload][tick_key][1]))
        print(curline + ' \\\\')
